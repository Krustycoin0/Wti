...
const SYMBOL = 'USOIL'; // Simbolo del petrolio Crude Oil (WTI) per Alpha Vantage
...
async function aggiornaGrafico(data, signal) {
    ...
    supportLevels.forEach(level => {
        const supportLine = chart.addLineSeries({ color: 'green', lineWidth: 1, lineStyle: 1 })
        supportLine.setData(dailyPrices.map((value, index) => ({ time: index + 1, value: level })))
    });
    resistanceLevels.forEach(level => {
        const resistanceLine = chart.addLineSeries({ color: 'red', lineWidth: 1, lineStyle: 1 })
    ...
    if (bollingerBands) {
        ...
        const upperBollingerLine = chart.addLineSeries({ color: 'rgba(144, 238, 144, 0.5)', lineWidth: 1, lineStyle: 2 })
        upperBollingerLine.setData(dailyPrices.map((value, index) => ({ time: index + 1, value: bollingerBands.upper })))

        const lowerBollingerLine = chart.addLineSeries({ color: 'rgba(144, 238, 144, 0.5)', lineWidth: 1, lineStyle: 2 })
    ...
    if (rsi) {
        ...
        const rsiSeries = chart.addLineSeries({ color: 'orange', lineWidth: 1, lineStyle: 1 });
    ...
    if (macd) {
        ...
        const macdSeries = chart.addLineSeries({ color: 'purple', lineWidth: 1, lineStyle: 1 });
        macdSeries.setData(dailyPrices.map((value, index) => ({ time: index + 1, value: macd.macd })));

        const signalSeries = chart.addLineSeries({ color: 'gray', lineWidth: 1, lineStyle: 1 });
    ...
    fiboLevels.forEach(level => {
        const fiboLine = chart.addLineSeries({ color: 'grey', lineWidth: 0.5, lineStyle: 3 })
